...
# map1['YP']=map1.YP.str.strip('>').str.split(' ').str[0]
...
#     jeff=j.split('/')[2]#jeff=os.path.basename(j).split('.')[0]
...
#     # kk.drop(columns=['count','evalue']).to_csv("/groups/cgsd/dcmorgan/CRC_data/no_duplicated/annot/"+SS+"_idx_table4.txt",header=True,index=False,sep='\t')
#     # kk.drop(columns=['count','evalue']).to_csv("/groups/cgsd/dcmorgan/PRJNA544527/V2_results/annot/"+SS+"_idx_table3.txt",header=True,index=False,sep='\t')
#     kk.drop(columns=['count','evalue']).to_csv("~/epilepsy/annot/"+SS+"_idx_table4.txt",header=True,index=False,sep='\t')

R=glob.glob('/groups/cgsd/dcmorgan/PRJNA544527/V2_results/annot/*_idx_table4.txt')
# R=glob.glob('/groups/cgsd/dcmorgan/CRC_data/no_duplicated/annot/*idx_table3.txt')
# R=glob.glob('~/epilepsy/annot/*idx_table4.txt')
for i,j in enumerate(R):
    ...
    if i!=0:
        ...
        BB=AA
# BB.fillna(0).to_csv('~/epilepsy/annot_contigs4.txt',sep='\t')
# BB.fillna(0).to_csv('/groups/cgsd/dcmorgan/CRC_data/no_duplicated/annot_contigs_3_mean.txt',sep='\t')
BB.fillna(0).to_csv('/groups/cgsd/dcmorgan/PRJNA544527/V2_results/annot_contigs_4_mean.txt',sep='\t')
